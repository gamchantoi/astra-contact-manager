...
{
    public class EntitySSHRepository : ISSHRepository
    {
        ...
        {
            _sshStream.Write("ppp secret print detail without-paging");
            var result = _sshStream.ReadResponse();
            return _helper.BuildPPPSecrets(result);
        }
        ...
        public string ppp_secret_add(PPPSecret secret)
        {
            ...
            command.Append(BuildCommand("name", secret.Name))
                    ...
                    .Append(BuildCommand("comment", secret.Comment))
                    .Append(BuildCommand("disabled", secret.Status.Equals(0).ToString().ToLower()));
